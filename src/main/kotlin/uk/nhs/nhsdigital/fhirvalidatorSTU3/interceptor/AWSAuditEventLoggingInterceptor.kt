package uk.nhs.nhsdigital.fhirvalidatorSTU3.interceptor
...
import uk.nhs.nhsdigital.fhirvalidatorSTU3.util.FhirSystems
...
class AWSAuditEventLoggingInterceptor(
...
)
 {
    ...
    fun createAudit(
        ...
        entityComponent.addDetail().setType("query").value = path.toByteArray()
        ...
        auditEvent.source
           ...
           .getType().add(Coding().setDisplay("Device"))


        // Agent Application
        val agentComponent = auditEvent.addAgent()
        agentComponent.requestor = true
        agentComponent.media = Coding().setSystem(FhirSystems.DICOM_AUDIT_ROLES).setCode("110150")
        ...
        if (patientId != null) {
            ...
            if (patientId.startsWith("Patient/")) {
                patient.reference = Reference().setReference(patientId)
            } else {
                patient.reference =
                    Reference()
